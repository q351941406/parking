<template>
  <div data-theme="cupcake" class="detail">
    ...
    <div
      v-if="isLoading"
      class="fixedbg-opacity-50 z-50 flex items-center justify-center"
    >
      <span class="loading loading-spinner loading-lg"></span>
    </div>
    <div class="mt-0 shadow-sm bg-white">
      <div
        class="carousel carousel-center p-0 space-x-1 bg-neutral w-full"
        id="gallery"
      >
        <a
          class="carousel-item flex items-end"
          v-for="(item, index) in data.scene_imgs"
          :key="index"
          :href="item.url"
    ...
    <div class="price-wrapper flex flex-col p-5 bg-white gap-2.5 shadow-sm">
      <div
        ...
        :class="{ 'border-primary': currentPrice == item.price }"
        @click="currentPrice = item.price"
      >
        <div class="left">
        ...
        <div class="left">
          <span class="stat-value text-accent text-2xl">
            ¥{{ item.price }}/天</span
          >
        </div>
      </div>
    ...
    <!-- Footer -->
    <div
      class="fixed inset-x-0 bottom-0 bg-white px-4 py-3 border-t border-gray-300"
    >
      <div class="flex justify-between items-center">
        <div class="flex-1 align-middle">
          <!-- <div class=" align-middle"> -->
          <span class="text mr-2 total-price"
            >总费用：
            <span class="stat-value text-accent text-2xl"
              >¥{{ currentPrice * currentPriceCount }}</span
            >
          </span>
          <!-- </div> -->
        </div>
        <div class="flex-1">
          <div class="items-center">
            <button
              class="btn btn-sm mx-1.5 btn-primary"
              :disabled="currentPriceCount == 1"
